...
|---|---|---|---:|
| `.py` / `.jl` | Readers and execution tools | Executable analysis, narrative, and generation logic | Yes |
| retained `.md` | Search and indexing tools | Executed evidence with original tables | No |
| `.ipynb` | Notebook users | Optional interactive projection of Python source | Usually synchronised |
| `.html` | Website readers | Polished browsing interface | No |
| embedding Markdown copy | Auditors and developers | Raw tables plus derived retrieval text | No |
| future SQLite/vector index | Search clients and applications | Rebuildable retrieval cache | No |
...
The [searchable evidence pipeline](searchable-evidence.md) explains how written documents, retained Markdown, table normalisation, database synchronisation, and embeddings fit together.
...
The raw table remains useful evidence. A reader can inspect column alignment, missing cells, duplicate ranks, inconsistent units, impossible values, and whether the normaliser itself introduced an error. The derived representation is easier to retrieve, but it is not treated as independent evidence.
