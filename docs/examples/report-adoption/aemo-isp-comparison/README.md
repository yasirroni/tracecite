...
Run these commands from the example's own directory (`docs/examples/report-adoption/aemo-isp-comparison`).
...
The report uses two citation routes:

1. **Local PDF citations with physical-page locators**: the 2024 ISP is cited at physical page 10, and the 2026 ISP is cited at physical pages 76 and 77. During authoring, these links resolve to the PDFs under `sources/aemo/`; the public snapshot rewrites them to the corresponding official AEMO URLs while preserving each `#page=N` fragment.

2. **Plain website citation**: the general AEMO ISP program page (https://www.aemo.com.au/energy-systems/major-publications/integrated-system-plan-isp) has no local file and is never rewritten; it appears unchanged in both authored and published output.
